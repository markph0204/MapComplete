...
class SlippyMap(QtCore.QObject):
    ...
    def render(self, p, rect):
        for x in range(self._tilesRect.width()):
            for y in range(self._tilesRect.height()):
                tp = Point(
                    x + self._tilesRect.left(), y + self._tilesRect.top())
                box = QtCore.QRect(self.tileRect(tp))
                if rect.intersects(box):
                    p.drawPixmap(
                        box, self._tilePixmaps.get(tp, self._emptyTile))

    def pan(self, delta):
        dx = QtCore.QPointF(delta) / float(TDIM)
        center = tileForCoordinate(
            self.latitude, self.longitude, self.zoom) - dx
    ...
    def download(self):
        ...
        path = 'https://mts2.google.com/vt?lyrs=y&x={0}&y={1}&z={2}'.format(
            grab.x(), grab.y(), self.zoom)
...
class MapZoom(QtGui.QMainWindow):
    ...
    def aboutOsm(self):
        QtGui.QDesktopServices.openUrl(
            QtCore.QUrl('http://www.openstreetmap.org'))
